...
public class SpeedTest extends TestContext {
  ...
  public void runTest( ) throws Exception {
    ...
    LOG.info( String.format( "file=%-20s count=%-5d min=%-10.5f max=%-10.5f avg=%-10.5f stdev=%-10.5f errbar=%-10.5f\n",
                             threadCount, total, min, max, 1000.0*(sum / total),
                             Math.sqrt( sumsq / total - ( sum / total ) * ( sum / total ) ),
                             1000.0*(sum / total)+Math.sqrt( sumsq / total - ( sum / total ) * ( sum / total ) )) );
  ...
  class DoDescribe implements Callable<List<Double>> {
    ...
    public List<Double> call( ) throws Exception {
      ...
      Jec2 myClient = new Jec2( System.getenv( "EC2_ACCESS_KEY" ), System.getenv( "EC2_SECRET_KEY" ), false, eucaUrl.getHost( ), eucaUrl.getPort( ) );
      myClient.setResourcePrefix( "/services/Eucalyptus" );
      myClient.setMaxConnections( 16 );
      myClient.setMaxRetries( 10 );
      MultiThreadedHttpConnectionManager connMgr = new MultiThreadedHttpConnectionManager();
      HttpConnectionManagerParams connParams = connMgr.getParams();
      connParams.setMaxTotalConnections(1);
      connParams.setMaxConnectionsPerHost(HostConfiguration.ANY_HOST_CONFIGURATION, 1);
      ...
      for( int i = 0; i < 5; i++ ) {
        myClient.describeInstances( EMPTY );
      }
      barrier.await( );
      for ( int i = 0; i < 100; i++ ) {
        long start = System.currentTimeMillis( );
